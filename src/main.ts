...
async function main(){
  ...
  hud.root.querySelectorAll<HTMLButtonElement>('[data-slot]').forEach(b=>{
    b.onpointerdown=e=>{e.preventDefault();if(!paused){advance();audio.unlock();sim.activateArt(Number(b.dataset.slot));b.setPointerCapture(e.pointerId);}b.blur();view.canvas.focus();};
  ...
  if(import.meta.env.DEV){Object.assign(window,{trinity:{sim,view,input,hud,audio,controls,tutorial,guild,pause:setPaused,persist,snapshot:()=>({state:sim.state.state,player:{...sim.player},enemies:sim.enemies.map(e=>({...e,hits:[...e.hits]})),counters:{...sim.counters},loadout:sim.loadout,stats:view.stats(),assets:view.loadedAssets,assetErrors:view.assetErrors,paused})}});}
}
