...
export interface SaveData { version: 3; progression:Progression; attributes: Attributes; loadout: (string|null)[]; settings: { quality: 'low'|'medium'|'high'; sensitivity: number; sound: boolean; timingMusic:boolean; bindings:Bindings }; counters: { kills: number; parries: number; breaks: number; perfects: number; arts: number }; }
export function defaultSave(): SaveData { return {version:3,progression:freshProgression(),attributes:{...defaultAttributes},loadout:['crescent-break',null,null,null],settings:{quality:'medium',sensitivity:1,sound:true,timingMusic:true,bindings:defaultBindings()},counters:{kills:0,parries:0,breaks:0,perfects:0,arts:0}}; }
export function migrateSave(raw: unknown): SaveData {
  ...
  try {if(Array.isArray(data.loadout)&&validLoadout(data.loadout,fallback.progression,'sword')&&data.loadout.every(id=>id===null||(typeof id==='string'&&!!arts[id]))) fallback.loadout=equipArts(data.loadout);} catch { /* Invalid loadouts recover to the starter Art. */ }
