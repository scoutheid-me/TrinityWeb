...
export class ControlsMenu {
  ...
  constructor(private input:GameInput,private pause:()=>void,private changed:()=>void){
    ...
    window.addEventListener('keydown',e=>{
      ...
      if(this.capture){e.preventDefault();e.stopImmediatePropagation();if(e.code==='Escape'){this.capture=null;this.render();this.status('Binding cancelled.');}else if(!e.repeat){if(e.ctrlKey||e.altKey||e.metaKey)this.status('Use a single key without Ctrl, Alt, or the Windows key.');else this.bind(e.code);}return;}
  ...
  open(){this.pause();this.input.suspended=true;this.input.clear();this.panel.hidden=false;this.render();(this.panel.querySelector('#controls-close') as HTMLButtonElement).focus();}
  ...
  private bind(code:string){
    ...
    this.input.bindings=result.bindings;this.capture=null;this.input.clear();this.changed();this.render();this.status(`${actionLabels[action]} → ${bindingLabel(code)}. Saved.`);
  ...
  render(){
    ...
    for(const action of actions){
      ...
      for(const slot of [0,1] as const){
        ...
        button.onclick=()=>{this.capture={action,slot};this.input.clear();this.render();this.status(`Listening for ${actionLabels[action]}. Escape cancels.`);};
