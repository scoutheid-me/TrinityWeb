...
export class HUD {
  ...
  update(view:LabScene){
    ...
    const threat=sim.flags.freezeAI?undefined:sim.enemies.filter(e=>e.pattern&&e.hp>0).map(e=>({enemy:e,at:e.attackStart+e.pattern!.hits.find((_,i)=>!e.hits.has(i))!})).filter(t=>Number.isFinite(t.at)).sort((a,b)=>a.at-b.at)[0];
    ...
    this.el('timing').style.opacity=this.showTiming&&(charging||!!art)?'1':'0';
    let remaining=0;
