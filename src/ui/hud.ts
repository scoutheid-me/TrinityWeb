...
export class HUD {
  ...
  setBindings(bindings:Bindings){
    ...
    this.root.querySelector('.intro-help')!.innerHTML=`Tap <b>${key('attack')}</b> for low damage and +10 SP on hit.<br>Earn <b>30 SP</b>. Hold <b>${key('art1')}</b> to charge Crescent Break.<br>Release <b>${key('art1')}</b> as the squares meet: Perfect 100%, Good 60%.<br>Miss: no strike; SP is spent. Cyan shows your hit area.<br>Follow the musical lead-in; act on the bright note.<br>Failed parry: <b>50% extra damage</b>.<br>Perfect Parry with <b>${key('parry')}</b>: +14 SP, zero damage.<br>Red sweep? Dodge with <b>${key('dodge')}</b>.`;
  }
  ...
  update(view:LabScene){
    ...
    const art=sim.art,charging=!!art&&art.grades[0]===null;
    this.el('timing').style.opacity=this.showTiming&&charging?'1':'0';
    let remaining=0;
    if(art&&charging){remaining=art.start+art.definition.nodes[0].at-sim.now;this.el('timing-title').textContent=art.definition.name.toUpperCase()+' · CHARGING';this.el('art-nodes').textContent='GOOD 60% · PERFECT 100% · MISS FAILS';}
    ...
    this.el('timing-action').textContent=art?`RELEASE ${bindingText(this.bindings,('art'+(art.slot+1)) as Action)} WHEN THE SQUARES MEET`:'';
    for(let i=0;i<4;i++){const id=sim.loadout[i],def=id?arts[id]:null,slot=this.el(`slot-${i}`);slot.querySelector('b')!.textContent=def?.name??'Empty slot';slot.querySelector('small')!.textContent=def?`${def.cost} SP · HOLD / RELEASE`:'UNEQUIPPED';slot.setAttribute('aria-label',`Art slot ${i+1}: ${def?.name??'Empty'}`);slot.classList.toggle('available',!!def&&(p.sp>=def.cost||sim.flags.infiniteSp));slot.classList.toggle('active',!!def&&sim.art?.definition.id===def.id);}
