...
export class HUD {
  ...
  constructor(public sim:CombatSimulation){
    this.root.innerHTML=`
      ...
      <section class="control-panel"><div><kbd>W A S D</kbd> Move <kbd>SHIFT</kbd> Sprint</div><div><kbd>LMB / J</kbd> Tap attack · build SP</div><div><kbd>SPACE</kbd> Dodge <kbd>Q</kbd> Counter</div><div><kbd>TAB</kbd> Lock <kbd>RMB</kbd> Orbit</div><div><kbd>F</kbd> Guard <kbd>R</kbd> Reset <kbd>\`</kbd> Lab tools</div></section></footer>
      <div id="performance"></div><div id="damage-layer"></div>
      <section id="overlay"><div class="intro"><span class="eyebrow">TRINITY / FIRST PLAYABLE</span><h1>Training Room</h1><p>Learn combat at the entrance orb. Earn Linear.</p><div class="intro-rule"></div><div class="intro-help">Hold <b>left mouse / J</b>, then release at the flash.<br>Earn <b>30 SP</b>. Press <b>1</b> for Crescent Break.<br>Tap <b>left mouse / J</b> on its single pulse.<br>Read gold attacks. Counter with <b>Q</b>.<br>Red sweep? Dodge with <b>Space</b>.</div><button id="begin" disabled>Preparing the hall…</button><small id="load-status">Loading original Blender assets</small></div></section>
  ...
  setBindings(bindings:Bindings){
    ...
    panel.innerHTML=`<div><kbd>${key('forward')} ${key('left')} ${key('backward')} ${key('right')}</kbd> Move <kbd>${key('sprint')}</kbd> Sprint</div><div><kbd>${key('attack')}</kbd> Tap attack · build SP</div><div><kbd>${key('dodge')}</kbd> Dodge <kbd>${key('parry')}</kbd> Counter</div><div><kbd>${key('lock')}</kbd> Lock <kbd>${key('orbit')}</kbd> Orbit</div><div><kbd>${key('guard')}</kbd> Guard <kbd>${key('reset')}</kbd> Reset <kbd>${key('debug')}</kbd> Lab tools</div>`;
    ...
    this.root.querySelector('.intro-help')!.innerHTML=`<p>Touch the entrance orb to start Ilyra’s Guild Combat Trial. Complete the combat basics to learn Linear.</p><details class="nested-info"><summary>Quick controls</summary><p>Tap ${key('attack')} to build SP. Hold an Art button and release on its bright note. ${key('parry')} counters; ${key('dodge')} dodges; ${key('lock')} locks a target. Perfect counters take no damage. Failed counters take 50% more.</p></details>`;
  }
  ...
  update(view:LabScene){
    ...
    if(e){this.root.querySelector('.enemy-title b')!.textContent=e.species==='boar'?'Woodland Boar':'Aether Sentinel';this.root.querySelector('.enemy-title span')!.textContent=e.species==='boar'?'TRAINING BEAST':'TRAINING CONSTRUCT';bar('enemy-hp',e.hp,e.maxHp);bar('enemy-break',e.break,100);this.el('enemy-values').textContent=`${Math.ceil(e.hp)} / ${e.maxHp}`;this.el('enemy-state').textContent=e.hp<=0?`DEFEATED · ${bindingText(this.bindings,'reset')} TO RESET`:e.state==='Broken'?'BROKEN · DAMAGE ×1.6':e.pattern?`${e.pattern.name.toUpperCase()} · ${e.pattern.parryable?'COUNTER OR DODGE':'DODGE'}`:e.state.toUpperCase();}
    const selected=sim.flags.freezeAI||sim.art?undefined:imminentThreat(sim);const threat=selected?{enemy:selected.enemy,at:selected.phase!.contactAt}:undefined;
    const defense=this.el('defense-cue');defense.hidden=!threat||!this.showTiming;
    if(threat){const parryable=threat.enemy.pattern!.parryable,remaining=threat.at-sim.now-(parryable?80:150);defense.classList.toggle('sweep',!parryable);defense.querySelector('i')!.style.transform='rotate(45deg) scale('+Math.max(.5,Math.min(2,1+remaining/600))+')';defense.querySelector('span')!.textContent=(remaining<=55?'NOW · ':'BUILD-UP · ')+(parryable?'COUNTER '+bindingText(this.bindings,'parry'):'DODGE '+bindingText(this.bindings,'dodge'));}
