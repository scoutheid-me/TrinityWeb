...
export class GuildInvitation {
 ...
 show(completed:boolean){this.panel.hidden=false;this.panel.innerHTML=`<div class="onboarding-card"><small>WARDEN ILYRA · GUILD HALL</small><h2 id="invitation-title">Guild Challenge</h2><p>Welcome to the Training Room. Will you take the Guild’s combat induction?</p><p>Learn basic attacks, dodging, perfect parries, charged Combat Arts and Break.</p><div class="challenge-reward"><b>${completed?'Already learned:':'Completion reward:'} Linear</b><p>Hold to charge, then release. Perfect drives a long 6 m piercing dash through the target; Good deals 60% damage and travels 1 m. Miss fails the Art.</p></div><button id="accept-guild-challenge">${completed?'Practice again':'Accept challenge'}</button><button id="decline-guild-challenge">Not now</button><small>You can return through Menu → Friends → Guild Challenge.</small></div>`;
