...
export class CombatTutorial {
  ...
  render(){
    ...
    this.panel.querySelector<HTMLButtonElement>('#tutorial-next')!.onclick=()=>{if(this.step===4&&this.completed){this.sim.progression.tutorialCompleted=true;this.sim.progression.learned.linear='induction';this.exit();this.sim.emit('notice','INDUCTION COMPLETE · Linear learned. Equip it with Edit Arts.');this.finish();}else{this.step++;this.prepare();}};
