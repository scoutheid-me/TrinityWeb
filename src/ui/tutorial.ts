...
export class CombatTutorial {
  ...
  start(guided=false){if(!this.active){this.facingSetting=this.sim.autoFaceTarget;this.weapon=this.sim.weapon;this.flags={...this.sim.flags};this.loadout=[...this.sim.loadout];}this.sim.practiceMode=true;this.sim.weapon='sword';this.sim.loadout=['focused-strike',null,null,null];this.active=true;this.guided=guided;if(guided)this.sim.autoFaceTarget=true;this.step=guided?-2:0;this.prepare();}
  ...
  private renderOrientation(k:(a:Parameters<typeof bindingText>[1])=>string){
    ...
    this.panel.innerHTML=`<small>GUILD CHALLENGE · ${lock?1:2} / 7</small><h2>${lock?'Target your opponent':'Move around your target'}</h2><p>${lock?'Press '+k('lock')+' to lock onto the Sentinel. The camera frames its body; '+k('switchTarget')+' cycles opponents.':'Hold '+k('left')+' or '+k('right')+' to move sideways while locked on. Your character and camera keep facing the target. Auto-facing can be disabled in Settings.'}</p><strong role="status">${this.completed?'Complete ✓':lock?'Lock onto the Sentinel.':'Move at least 0.8 m sideways while locked on.'}</strong><div><button id="tutorial-next" ${this.completed?'':'disabled'}>Next lesson</button><button id="tutorial-exit">Exit tutorial</button></div>`;
  ...
  render(){
    ...
    if(this.guided){lessons[2]=['Parry & counterattack',`Press ${k('parry')} just before impact. A Perfect Parry takes zero damage and builds SP. Your Perfect Parry automatically returns a counter hit at 2× weapon base damage, scaled by Strength. Follow up during the opening. A mistimed parry takes 50% extra damage.`, 'Perfect Parry to damage the attacker.'];}
