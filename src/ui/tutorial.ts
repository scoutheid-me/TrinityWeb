...
export class CombatTutorial {
  ...
  prepare(){
    ...
    s.enemies[0].nextPattern=this.step===1?2:0;
  ...
  observe(events:CombatEvent[]){
    ...
    if(this.step===1||this.step===2)s.enemies[0].nextPattern=this.step===1?2:0;
  ...
  render(){
    ...
    const lessons=[
      ...
      ['Perfect Parry',`Face the sentinel. The higher rising chime warns of a parryable strike. Press ${k('parry')} on its final note, just before impact. Your first 170 ms parry: zero damage, +14 SP, +32 Break. An early or late attempt can be hit. ${k('guard')} guards for reduced damage at a stamina cost.`, 'Perfect Parry one gold attack.'],
      ['Combat Arts & musical timing',`Practice SP is supplied. Press ${k('art1')} for Crescent Break (30 SP). Tap ${k('attack')} when each moving square meets its outline and the bright note sounds. Three inputs lead to an automatic finisher. Perfect is ±55 ms; Good is ±120 ms. Misses are weaker. Basics remain untimed.`, 'Land a Good or Perfect Art strike.'],
      ['Break & the combat loop',`This sentinel starts at 80 Break. Use ${k('art1')} and its timed taps to fill the meter. At 100 Break the enemy staggers for 3.4 seconds and takes ×1.6 damage. In free combat: build SP with basics or parries, spend it on Arts, then punish the opening. Recover stamina between defenses.`, 'Break the sentinel.'],
    ];
