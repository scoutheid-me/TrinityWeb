...
export function installGlossary(){
 const popup=document.createElement('aside');popup.id='context-help';popup.hidden=true;popup.setAttribute('role','tooltip');document.getElementById('ui')!.append(popup);let pinned:HTMLElement|null=null;
 const show=(el:HTMLElement)=>{popup.textContent=el.dataset.help!;popup.hidden=false;const r=el.getBoundingClientRect();popup.style.left=Math.max(12,Math.min(innerWidth-292,r.left))+'px';popup.style.top=Math.min(innerHeight-130,r.bottom+10)+'px';};
