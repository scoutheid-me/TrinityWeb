...
export class CombatAudio {
  ...
  timingMusic=true;cueVolume=.8;effectsVolume=.7;cueOffsetMs=0;
  ...
  syncTiming(sim:CombatSimulation,paused:boolean,rate=1){
    ...
    const id=`${phrases.map(p=>p.id).join('|')}:${rate}:${this.cueVolume}:${this.cueOffsetMs}`;if(this.phraseId===id)return;this.stopTiming();this.phraseId=id;
    const ctx=this.context;
    for(const note of phrases.flatMap(p=>p.notes)){
      if(note.at<sim.now-40)continue;
      const when=ctx.currentTime+Math.max(0,(note.at-sim.now)/1000/rate-this.cueOffsetMs/1000);
      ...
      for(const harmonic of (note.chord?[1,1.2,1.5,2]:[1,2])){
        ...
        gain.gain.setValueAtTime(0,when);gain.gain.linearRampToValueAtTime(this.cueVolume*(note.level??(note.accent?.065:.027))/(harmonic*(note.chord?1.7:1)),when+.008);gain.gain.exponentialRampToValueAtTime(.0001,when+duration);
  ...
  tone(frequency:number,duration:number,volume=.06,type:OscillatorType='sine'){
    if(!this.enabled||!this.context||this.effectsVolume<=0)return;
    ...
    gain.gain.setValueAtTime(volume*this.effectsVolume,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(.001,ctx.currentTime+duration);osc.connect(gain);gain.connect(ctx.destination);osc.start();osc.stop(ctx.currentTime+duration);
  }
  swish(){
    ...
    const source=ctx.createBufferSource(),filter=ctx.createBiquadFilter(),gain=ctx.createGain();source.buffer=buffer;filter.type='highpass';filter.frequency.value=1400;gain.gain.value=.07*this.effectsVolume;
  ...
  event(event:CombatEvent){if(event.type==='hit')this.tone(event.target==='player'?90:event.weakPoint?520:170,.12,.07,'triangle');if(event.type==='parry')this.tone(1100,.28,.07);if(event.type==='break')this.tone(240,.45,.09,'triangle');if(event.type==='dodge')this.swish();if(event.type==='slash')this.tone(event.target?125:290,.07,.018,'triangle');}
}
