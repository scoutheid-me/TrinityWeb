export interface Profile {name:string;created:boolean;character:'man'|'woman';potions:{health:number;stamina:number};pockets:(string|null)[];explored:string[];sentinelSeen:boolean;boarSeen:boolean;chats:string[];}
...
export function freshProfile():Profile{return {name:'Wayfarer',created:false,character:'man',potions:{health:2,stamina:2},pockets:['health','stamina',null,null,null],explored:[],sentinelSeen:false,boarSeen:false,chats:[]};}
export function validateProfile(raw:unknown):Profile{
 ...
 p.sentinelSeen=r.sentinelSeen===true;p.boarSeen=r.boarSeen===true;if(Array.isArray(r.chats))p.chats=[...new Set(r.chats.filter(v=>['guild','oath'].includes(v)))];return p;
}
