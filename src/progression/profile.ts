export interface Profile {name:string;potions:{health:number;stamina:number};pockets:(string|null)[];explored:string[];sentinelSeen:boolean;chats:string[];}
...
export function freshProfile():Profile{return {name:'Wayfarer',potions:{health:2,stamina:2},pockets:['health','stamina',null,null,null],explored:[],sentinelSeen:false,chats:[]};}
export function validateProfile(raw:unknown):Profile{
 const p=freshProfile();if(!raw||typeof raw!=='object')return p;const r=raw as Partial<Profile>;p.name=cleanName(r.name);
