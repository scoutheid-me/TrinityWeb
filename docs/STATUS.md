...
## Current implementation — September 23, recommendations 1–3

This section supersedes the historical balance and completion statements above. Greatsword now leads measured damage and Break: base 28, basic Break 9, shared Art Break multiplier 1.6, 320 ms startup and 400 ms recovery. Rapier remains faster with base 10 / Break 3 / Art Break 1.1; sword uses base 8 single or 6 group / Break 2. Counter costs 16 stamina and still returns weapon-scaled damage, prevents the incoming hit, and penalizes failed commitment by 50%. The 30-second stationary basic benchmark measures sword 21.3 single / 34.1 group DPS, rapier 38.6 / 38.6, greatsword 40.8 / 122.5. Group totals reflect actual coverage. Perfect shared-Art scenarios also put greatsword first. These scripted policies are balance checks, not human playtest outcomes.

Sentinel, Boar and player reactions use new original Blender-authored additive body curves with contact/recovery markers. Both greatsword grips remain constrained. Original models still render; this is prototype rigid-part animation, not a finished production character pipeline.

Public playtest builds omit GM/debug entry points and ignore GM overrides. Added comfort settings, calibration, safe save preview/import/recovery, bounded opt-in diagnostic download, and optional encounter recap. Tools/package_playtest.ps1 builds a static hosting ZIP. There is no external hosted URL yet. See PLAYTEST_HANDOFF.md for distribution and feedback instructions.

Validation: 123 unit tests passed. The previous 30 browser scenarios passed across the full run and corrected targeted reruns; the new duel-pose browser scenario also passed, for 31 scenarios covered. Both first/third-person greatsword grip cycles passed. Public-build save/import/settings checks and production startup with remote requests blocked passed. Both build modes pass with the existing large-bundle warning. Inspected Sentinel and first-person Boar screenshots show original assets and attack poses. One short automated WebGPU sample measured 102 FPS / 10 ms frames with 458 draw calls; this is not a hardware performance guarantee. Human combat feel, muted play, sustained low-end performance, continuous blade contact and production animation remain unvalidated or unfinished.
