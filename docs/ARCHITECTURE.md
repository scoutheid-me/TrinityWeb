...
The location display is Training Room, nested under Guild Hall / Town of Beginnings. Entry leaves an empty room and directs the player to the entrance orb. The orb starts the seven-stage Guild Combat Trial (two orientation stages and five combat lessons). Completion grants Linear and presents its SP cost, hold timing and 6 m Perfect / 1 m Good travel before offering the Art editor. The duplicate Guild trial chain is retired from the UI. Interact remains a saved/remappable action (E by default, T switches targets) and contextual prompts use the current binding.
