...
test('built game starts without runtime CDN requests and omits dev hooks',async({page})=>{
 ...
 await page.goto('/');await expect(page.locator('#begin')).toBeEnabled({timeout:30000});
 await page.locator('#begin').click();await expect(page.locator('#overlay')).toBeHidden();
 await expect(page.locator('#enemy-hud')).toBeHidden();await page.keyboard.press('m');await page.locator('#friends-menu').click();await page.locator('#personal-guild').click();await page.locator('[data-challenge="positioning"]').click();await page.keyboard.press('Tab');await expect(page.locator('#lock-label')).toHaveCSS('opacity','1');
