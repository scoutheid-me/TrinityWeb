...
describe('Guild progression',()=>{
 ...
 it('earns Footwork through real basics and a timed dodge, then preserves its reward',()=>{const s=new CombatSimulation();s.beginChallenge('positioning');close(s);for(let i=0;i<3;i++){s.pressAttack();tick(s,500);}tick(s,530);s.dodge();tick(s,200);expect(s.encounter.result).toBe('completed');expect(s.progression.learned['aether-step']).toBe('positioning');expect(s.encounter.stats.damageTaken).toBe(0);const data={...defaultSave(),progression:s.progression,loadout:['crescent-break','aether-step',null,null]};expect(migrateSave(JSON.parse(JSON.stringify(data))).loadout).toEqual(data.loadout);});
 ...
 it('requires and consumes a recent counter opportunity without charging on refusal',()=>{const s=new CombatSimulation();awardChallenge(s.progression,'positioning',false);awardChallenge(s.progression,'breaking',false);awardChallenge(s.progression,'countering',false);s.loadout=['stillwater-return',null,null,null];s.player.sp=50;expect(s.activateArt(0)).toBe(false);expect(s.player.sp).toBe(50);close(s);s.parry();s.receiveAttack(s.enemies[0],sentinelPatterns[0]);expect(s.player.hp).toBe(s.hpMax);expect(s.activateArt(0)).toBe(true);expect(s.lastParryAt).toBe(-Infinity);expect(s.player.sp).toBe(49);});
 it('executes mastery policies without mutating shared Art definitions',()=>{const s=new CombatSimulation();s.player.sp=100;s.progression.masteryChoice='break';s.activateArt(0);expect(s.art!.definition.nodes[0].break).toBe(54);expect(s.art!.definition.recovery).toBe(460);s.reset();s.player.sp=100;s.progression.masteryChoice='recovery';s.activateArt(0);expect(s.art!.definition.nodes[0].break).toBe(40);expect(s.art!.definition.recovery).toBe(200);});
...
it('completes the authored progression with live AI and action rules',()=>{
 ...
 expect(Object.keys(s.progression.learned)).toHaveLength(4);
 expect(s.progression.masterySources.length).toBeGreaterThanOrEqual(2);
});
