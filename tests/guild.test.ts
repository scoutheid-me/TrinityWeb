...
describe('Guild progression',()=>{
 ...
 it('earns Footwork through real basics and a timed dodge, then preserves its reward',()=>{const s=combatFixture();s.beginChallenge('positioning');close(s);for(let i=0;i<3;i++){s.pressAttack();tick(s,500);}tick(s,530);s.dodge();tick(s,200);expect(s.encounter.result).toBe('completed');expect(s.progression.learned['aether-step']).toBe('positioning');expect(s.encounter.stats.damageTaken).toBe(0);const data={...defaultSave(),progression:s.progression,loadout:['focused-strike','aether-step',null,null]};expect(migrateSave(JSON.parse(JSON.stringify(data))).loadout).toEqual(data.loadout);});
 it('practice modifiers and resets cannot grant rewards',()=>{const s=combatFixture();s.beginChallenge('positioning');s.flags.invulnerable=true;s.update(1);for(let i=0;i<3;i++)s.encounter.record({kind:'basic-hit',actor:'player',target:'a',attackId:i,phase:'basic',at:i,amount:5});s.encounter.record({kind:'evade',actor:'player',target:'a',attackId:4,phase:'0',at:4,amount:0});s.update(1);expect(s.encounter.result).toBe('completed');expect(s.progression.completed).toEqual([]);s.beginChallenge('positioning');s.reset();expect(s.encounter.result).toBe('abandoned');expect(s.progression.completed).toEqual([]);});
 ...
 it('keeps Stillwater a simple learned cut with no counter prerequisite',()=>{const s=combatFixture();for(const id of ['positioning','breaking','countering'] as const)awardChallenge(s.progression,id,false);s.loadout=['stillwater-return',null,null,null];s.player.sp=50;expect(s.activateArt(0)).toBe(true);expect(s.player.sp).toBe(32);expect(s.art!.definition.nodes).toHaveLength(1);});
 it('executes mastery policies without mutating shared Art definitions',()=>{const s=combatFixture();s.progression.learned['crescent-break']='legacy';s.loadout=['crescent-break',null,null,null];s.player.sp=100;s.progression.masteryChoice='break';s.activateArt(0);expect(s.art!.definition.nodes[0].break).toBe(54);expect(s.art!.definition.recovery).toBe(460);s.reset();s.player.sp=100;s.progression.masteryChoice='recovery';s.activateArt(0);expect(s.art!.definition.nodes[0].break).toBe(40);expect(s.art!.definition.recovery).toBe(200);});
 it('coordinates two enemies and separates their bodies',()=>{const s=combatFixture();for(const id of ['positioning','breaking','countering'] as const)awardChallenge(s.progression,id,false);s.beginChallenge('trial');s.player.z=0;for(const e of s.enemies){e.x=0;e.z=2;e.until=0;}tick(s,500);expect(s.enemies.filter(e=>e.pattern)).toHaveLength(1);expect(Math.hypot(s.enemies[0].x-s.enemies[1].x,s.enemies[0].z-s.enemies[1].z)).toBeGreaterThan(1.2);});
 it('does not reward empty Art swings or distant dodges',()=>{const s=combatFixture();s.beginChallenge('positioning');s.player.sp=100;s.activateArt(0);tick(s,620);s.releaseArt(0);tick(s,600);expect(s.encounter.stats.artHits).toBe(0);expect(s.encounter.stats.crescentHit).toBe(false);});
});
