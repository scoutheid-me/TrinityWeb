...
it('migrates old profiles and validates names, inventory and exploration',()=>{
 ...
 expect(p.name).toBe('Aria');expect(p.potions).toEqual({health:0,stamina:3});expect(p.pockets).toEqual(['health',null,'stamina',null,null]);expect(p.explored).toEqual(['0,0']);expect(p.chats).toEqual(['guild']);expect(migrateSave({...defaultSave(),profile:p}).profile).toEqual(p);
});
