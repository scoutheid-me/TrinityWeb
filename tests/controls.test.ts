...
describe('control profiles',()=>{
 ...
 it('rejects conflicting primary and alternate inputs',()=>{const before=defaultBindings();expect(assignBinding(before,'dodge',0,'KeyJ').error).toContain('Attack');expect(assignBinding(before,'attack',0,'KeyJ').error).toBeTruthy();});
...
describe('new combat loop',()=>{
 ...
 const s=new CombatSimulation();s.flags.freezeAI=true;s.player.z=0;s.enemies[0].z=2;s.pressAttack();
