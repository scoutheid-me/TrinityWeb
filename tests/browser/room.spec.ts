...
test('empty hall, physical rack, two-handed grip, live menu and camera preferences',async({page})=>{
 ...
 await expect(page.locator('#weapon-rack')).toBeVisible();await page.locator('[data-take-weapon="greatsword"]').click();await page.locator('#rack-close').click();
 ...
 await startCombatTrial(page);expect(await page.evaluate(()=>window.trinity.sim.enemies.length)).toBe(1);
