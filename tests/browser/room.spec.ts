...
test('empty hall, physical rack, two-handed grip, live menu and camera preferences',async({page})=>{
 ...
 await page.keyboard.press('g');await expect(page.locator('#weapon-rack')).toBeHidden();await expect(page.locator('#edit-arts')).toHaveCount(0);await page.keyboard.press('m');await page.locator('#character-menu').click();await page.locator('[data-category=arts]').click();await page.locator('#personal-edit-arts').click();await expect(page.locator('#rack-weapon')).toHaveCount(0);await page.locator('#tray-close').click();await page.keyboard.press('m');
 ...
 await page.locator('#menu').click();await page.locator('#open-controls').click();await page.locator('#auto-face-target').check();await page.locator('#controls-close').click();await page.locator('#begin').click();await page.waitForTimeout(500);
