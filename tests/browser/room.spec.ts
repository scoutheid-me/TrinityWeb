...
test('empty hall, physical rack, two-handed grip, live menu and camera preferences',async({page})=>{
 ...
 await page.keyboard.press('g');await expect(page.locator('#weapon-rack')).toBeHidden();await page.locator('#edit-arts').click();await expect(page.locator('#rack-weapon')).toHaveCount(0);await page.locator('#tray-close').click();
 ...
 await page.waitForTimeout(800);await page.keyboard.press('m');await page.locator('#perspective').click();await page.locator('#open-controls').click();
 await expect(page.locator('#controls-menu')).toHaveClass(/personal-live/);const position=await page.evaluate(()=>({...window.trinity.sim.player}));const before=await page.evaluate(()=>window.trinity.sim.now);await page.keyboard.down('w');await page.waitForTimeout(300);await page.keyboard.up('w');expect(await page.evaluate(()=>window.trinity.sim.now)).toBeGreaterThan(before+100);expect(await page.evaluate(p=>Math.hypot(window.trinity.sim.player.x-p.x,window.trinity.sim.player.z-p.z),position)).toBeGreaterThan(.2);
 await page.screenshot({path:'test-results/first-person-live-controls.png'});await page.locator('#auto-face-target').uncheck();await page.locator('#controls-close').click();
 ...
 await page.locator('#open-controls').click();await page.locator('#auto-face-target').check();await page.locator('#controls-close').click();await page.waitForTimeout(500);
