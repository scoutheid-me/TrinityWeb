...
test('rack displays real weapons, comparison data and persistent specialty Arts',async({page})=>{
 ...
 await page.keyboard.press('e');await expect(page.locator('#weapon-rack')).toContainText('400 ms recovery');await expect(page.locator('#weapon-rack')).toContainText('140 ms recovery');await page.screenshot({path:'docs/screenshots/weapon-comparison.png'});
 for(const [weapon,skill] of [['rapier','needle-step'],['greatsword','iron-horizon']]){await page.locator(`[data-take-weapon=${weapon}]`).click();expect(await page.evaluate(id=>window.trinity.sim.progression.learned[id],skill)).toBe('rack practice');}
